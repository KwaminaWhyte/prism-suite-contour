//! Font enumeration + resolution for the Type tool.
//!
//! Contour lays out text into real glyph outlines from a TrueType face
//! ([`crate::text`]). This module is the bridge between a *font-family name* (what
//! the user picks in the Character panel and what is persisted per text object)
//! and the *raw face bytes* that [`crate::text::layout`] parses with
//! [`ttf_parser`].
//!
//! Two pieces of process-wide state, built lazily on first use and shared by every
//! text object (layout is a free function called from the document model, the
//! editor, export, and tests):
//!
...
//!
//! Resolution is deliberately forgiving: an unknown / missing family always falls
//! back to the bundled [`Ubuntu Light`](DEFAULT_FONT_FAMILY) face, so old
//! `.contour` files (which carry no family) and files referencing a font absent on
//! this machine both render rather than disappearing.

use std::collections::HashMap;
use std::sync::{Arc, OnceLock, RwLock};

/// The bundled default font (Ubuntu Light, Ubuntu Font Licence — see
/// `assets/fonts/UFL.txt`). Embedded so a fresh install — and any document that
/// names no family or an unavailable one — always has a usable face.
pub static DEFAULT_FONT: &[u8] = include_bytes!("../assets/fonts/Ubuntu-Light.ttf");

/// The family name of the bundled [`DEFAULT_FONT`]. This is the default for new
/// text objects and the guaranteed fallback target. Kept in sync with the
/// embedded face's `name` table (Ubuntu Light's typographic family is "Ubuntu").
...
/// Character panel's family dropdown lists.
///
...
/// Resolve a (possibly absent / unknown) family name to concrete face bytes,
/// caching the result. `None` (old files, new objects) and any family the database
/// can't supply both resolve to the bundled [`DEFAULT_FONT`] — resolution never
/// fails, so text always renders.
pub fn resolve(family: Option<&str>) -> FaceBytes {
    let key = family.unwrap_or(DEFAULT_FONT_FAMILY);

    if let Some(hit) = cache().read().ok().and_then(|c| c.get(key).cloned()) {
        return hit;
    }

    let bytes = load_uncached(key);
    if let Ok(mut c) = cache().write() {
        c.entry(key.to_string()).or_insert_with(|| bytes.clone());
    }
    bytes
}
...
fn load_uncached(family: &str) -> FaceBytes {
    // The bundled family short-circuits to the embedded bytes — no file I/O, and
    // it works even when the OS font dir is unreadable.
    if family == DEFAULT_FONT_FAMILY {
        // Prefer a system-installed "Ubuntu" if present (matches what the user
        // sees elsewhere); else use the embedded copy.
        return query_source(family)
...
mod tests {
    ...
    /// `None` (old files / new objects) resolves to a parseable face — the bundled
    /// default.
    #[test]
    fn none_resolves_to_a_parseable_face() {
        let face = resolve(None);
        assert!(
            ttf_parser::Face::parse(&face.data, face.index).is_ok(),
            "the fallback face parses"
        );
    }
    ...
    fn unknown_family_falls_back() {
        let face = resolve(Some("This Font Does Not Exist 12345"));
        assert!(
            ttf_parser::Face::parse(&face.data, face.index).is_ok(),
    ...
    fn default_family_is_available_and_usable() {
        ...
        let face = resolve(Some(DEFAULT_FONT_FAMILY));
    ...
    fn resolution_is_cached() {
        let a = resolve(Some(DEFAULT_FONT_FAMILY));
        let b = resolve(Some(DEFAULT_FONT_FAMILY));
        assert!(
